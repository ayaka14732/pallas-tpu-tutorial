...
本章我们将深入解析它们的语义，并学习如何处理更复杂的切分模式。

## 循环的心智模型 (The Mental Model)

在 Pallas 中，你可以把 `pallas_call` 想象成一个巨大的 `for` 循环。
...
for i in range(X):
    for j in range(Y):
        # 1. 根据 in_spec 计算输入切片位置
        in_slice = in_spec.compute_slice(i, j)
        in_ref = input_array[in_slice]
        
        # 2. 根据 out_spec 计算输出切片位置
        out_slice = out_spec.compute_slice(i, j)
        out_ref = output_array[out_slice]
        
        # 3. 执行 Kernel（此时数据在 VMEM 中）
        kernel_fn(in_ref, out_ref)
```

在 TPU 上，这个网格 `(X, Y)` 保证是**按字典序严格顺序执行**的（即先遍历 `i`，再遍历 `j`）。
...
```
此时传入 Kernel 的 `in_ref` 形状将是 `(128,)`，而不是 `(1, 128)`。

## 越界填充 (Out-of-Bounds Padding)

如果原数组的大小不能被 `block_shape` 整除怎么办？

Pallas 会**自动处理越界读取**。当切片超出原数组边界时，Pallas 会自动在 VMEM 中填充默认值（通常是 0）。
...
掌握了 Grid 和 BlockSpec，你就可以自由地将任何大型张量切片送入 TPU 的超快 VMEM 中。下一章，我们将详细探讨 TPU 的内存空间标注。
