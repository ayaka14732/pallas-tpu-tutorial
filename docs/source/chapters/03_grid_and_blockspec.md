...
如果把第 4 章的内存空间看作“数据在哪里”，那么本章就是“数据按什么节奏流进去”。后续的矩阵乘法、流水线、RMSNorm、FlashAttention 和 Ragged Paged Attention，本质上都是在重新安排这两件事。
...
默认 `grid=()` 表示只执行一次。`grid=4` 会被规范化为 `grid=(4,)`。
...
需要注意：在 kernel 里不要用 Python `if i == 0:` 来判断 `program_id`，因为 `i` 是被 trace 的值。应使用 `pl.when`、`jax.lax.cond` 或 Pallas 支持的控制流。
...
因此，`grid=(pl.cdiv(n, block_size),)` 是常见写法，但不等于“可以放心把 padding 当 0 用”。
...
默认整数 `block_shape` 会被解释为 `pl.Blocked(size)`，也就是 `index_map` 返回块编号。Pallas 还提供几种特殊 BlockDim。
...
当你写 `pl.BlockSpec(memory_space=pltpu.HBM)` 且不指定 `block_shape/index_map` 时，kernel 收到的是整个数组的 HBM Ref。它不会被自动搬进 VMEM，需要用第 4 章的 `pltpu.sync_copy` 或 `pltpu.make_async_copy` 手动搬运。
...
### pl.Blocked：显式 blocked 维度

整数 block shape 会被规范化为 `pl.Blocked(size)`。下面两种写法等价：
...
x = jnp.arange(8, dtype=jnp.float32)
int_spec = pl.BlockSpec((4,), lambda i: (i,))
blocked_spec = pl.BlockSpec((pl.Blocked(4),), lambda i: (i,))
...
y1 = pl.pallas_call(
    ...
    out_specs=blocked_spec,
    grid=(2,),
    interpret=True,
)(x)

assert jnp.all(y0 == y1)
assert jnp.all(y1 == x * 3)
print(y1)
```

通常直接写整数即可；`pl.Blocked` 主要用于强调语义或和其他 `BlockDim` 并列。
...
### BlockSpec(memory_space=...)：指定 Ref 内存空间

`memory_space` 是 `BlockSpec` 的关键字参数。跨后端的 `pl.ANY` 可以表示“不限制/默认主存语义”；TPU 后端还会使用 `pltpu.HBM`、`pltpu.VMEM`、`pltpu.SMEM` 等更具体的空间。
...
whole_any = pl.BlockSpec(memory_space=pl.ANY)
...
assert jnp.all(y == x + 7)
print(y)
```

在 TPU 上写 `pl.BlockSpec(memory_space=pltpu.HBM)` 时，kernel 收到的是 HBM Ref，通常要在 kernel 内部手动 DMA；完整模式见第 4 章。
...
### scratch_shapes：给 kernel 传临时 Ref

`scratch_shapes` 分配不对应输入/输出的临时 Ref。参数顺序是：输入 Ref、输出 Ref、scratch Ref。
...
y = pl.pallas_call(
    ...
    in_specs=[spec],
    out_specs=spec,
    scratch_shapes=[pl.ANY((4,), x.dtype)],
    grid=(2,),
    interpret=True,
)(x)

assert jnp.all(y == x * 2 + 1)
print(y)
```

TPU 上更常见的写法是 `pltpu.VMEM(...)`、`pltpu.SMEM(...)` 或 `pltpu.SemaphoreType.DMA(...)`，第 4 章会展开。
...
这几个工具不是 kernel primitive，而是写 grid 和 tile 时常用的 Python/JAX 工具。
