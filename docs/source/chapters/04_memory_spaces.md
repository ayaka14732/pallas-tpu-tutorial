...
在前面的章节中，我们一直默认输入数据在 HBM 中，而 Kernel 内部操作的是 VMEM 中的引用。但在复杂的算子（如 FlashAttention 或 MatMul）中，我们往往需要显式地控制数据存放的内存层级。
...
- `pltpu.VMEM`：向量内存，用于存放向量算术单元的输入输出。
- `pltpu.SMEM`：标量内存，用于存放控制流变量和动态索引。
- `pltpu.HBM`：高带宽主存。
- `pltpu.CMEM`：公共内存（Common Memory，在某些 TPU 代次中可用）。
- `pl.ANY`：让编译器自行决定最佳位置。
...
如果你指定了 `memory_space=pltpu.VMEM`，编译器将**不会**生成 HBM 到 VMEM 的 DMA 拷贝指令，它期望传入 `pallas_call` 的对应参数本身就是一个驻留在 VMEM 中的引用。
...
这些中间结果不需要写回 HBM，它们只在 Kernel 执行期间存在于 VMEM 中。Pallas 提供了 `scratch_shapes` 参数来分配这些缓冲区。

### 分配 Scratch Buffers

在 `pallas_call` 的 `grid_spec`（或直接作为 `pallas_call` 的参数，取决于 API 版本）中，你可以传入一个 `scratch_shapes` 列表：
...
### Scratch Buffer 的生命周期

**极其重要的一点：** Scratch Buffer 的生命周期是**整个 Grid 执行期间**。

这意味着，当 Grid 从 `i=0` 推进到 `i=1` 时，Scratch Buffer 中的数据**会被保留**。这使得我们可以利用它在不同的 Grid 迭代之间传递状态！

最经典的例子就是沿着归约维度（Reduction dimension）进行累加：
如果 Grid 是 `(M, N, K)`，其中 `K` 是归约维度，那么对于相同的 `(M, N)`，随着 `K` 的增加，我们可以不断地向同一个 `acc_ref` 中累加数据，直到 `K` 遍历完毕，再将 `acc_ref` 写回 HBM。
